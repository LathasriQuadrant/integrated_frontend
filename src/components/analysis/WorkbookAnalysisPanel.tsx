...
const WorkbookAnalysisPanel = ({ bundle, usage, complexity, sharedTableNames }: WorkbookAnalysisPanelProps) => {
  ...
  // ✅ ADDED LOGGING - compares what the /analyze response actually sent for
  // this workbook against what survives the merge, so we can tell whether
  // "missing" tables/relationships/columns are missing from the backend
  // response itself, or getting dropped by the merge-by-name step here.
  useEffect(() => {
    console.log("🔵 WorkbookAnalysisPanel bundle.data_model RAW:", {
      workbook: bundle.workbook_metadata?.name,
      rawTableCount: bundle.data_model?.tables?.length ?? 0,
      rawTables: bundle.data_model?.tables,
      relationshipCount: bundle.data_model?.relationships?.length ?? 0,
      relationships: bundle.data_model?.relationships,
      datasourceCount: bundle.data_model?.datasources?.length ?? 0,
    });
  }, [bundle]);

  useEffect(() => {
    console.log("🔵 WorkbookAnalysisPanel mergedTables AFTER merge:", {
      mergedCount: mergedTables.length,
      mergedTables,
    });
  }, [mergedTables]);
