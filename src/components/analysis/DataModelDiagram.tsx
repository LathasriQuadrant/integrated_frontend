...
const DataModelDiagram = ({ tables, relationships, sharedTableNames }: DataModelDiagramProps) => {
  ...
  // ✅ ADDED LOGGING - surfaces exactly what this component received, and
  // flags the two silent failure modes: (1) a relationship whose
  // left_table/right_table doesn't exactly string-match any table name, so
  // its line is dropped with no error, and (2) a node that has zero column
  // info because it only ever appeared as a relationship endpoint, never in
  // the `tables` array itself.
  useEffect(() => {
    console.log("🟢 DataModelDiagram received:", {
      tableCount: tables.length,
      relationshipCount: relationships.length,
      tableNames: tables.map(tableLabel),
      nodeCount: nodeNames.length,
    });

    const nodeNameSet = new Set(nodeNames);
    const lowerToActual = new Map(nodeNames.map((n) => [n.toLowerCase().trim(), n]));
    ...
    relationships.forEach((r, i) => {
      [
        ...
        if (!value) {
          console.warn(`🟢 relationship[${i}] missing ${side}`, r);
          return;
        }
        if (!nodeNameSet.has(value)) {
          const closeMatch = lowerToActual.get(value.toLowerCase().trim());
          console.warn(
            `🟢 relationship[${i}] ${side}="${value}" does NOT exactly match any table name - this line will be silently dropped`,
            {
              relationship: r,
              closeMatchFound: closeMatch ?? null,
              hint: closeMatch
                ? `A near-match exists ("${closeMatch}") - likely a casing/whitespace/schema-prefix mismatch between the relationship pass and the table/schema pass.`
                : "No near-match found at all - this table may be missing from data_model.tables entirely.",
            },
          );
        }
      });
    ...
    tables.forEach((t) => {
      const label = tableLabel(t);
      if (label && !(t.columns && t.columns.length)) {
        console.warn(`🟢 table "${label}" has no columns in the response`, t);
      }
    });
  }, [tables, relationships, nodeNames]);
