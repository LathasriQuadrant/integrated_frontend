...
//       // ── Job completion: this is the ONLY place DatasourceName/DatasetId get
//       //    written, and only after report.save() has actually succeeded ──
//       if (currentJobId) {
//         try {
//           const jobDatasourceName = sessionStorage.getItem("current_datasource_name") || "";
//           // Prefer the value Migration.tsx set (covers both reuse and fresh-deploy
//           // paths); fall back to the dataset the report was embedded with, so we
//           // never write an empty DatasetId if that step didn't populate it.
//           const jobDatasetId = sessionStorage.getItem("current_dataset_id") || datasetId || "";
...
//               CompletedAt: new Date().toISOString(),
//               DatasourceName: jobDatasourceName,
...
export default function PowerBIReport() {
  ...
  // True only if this deployment went through the Lakehouse path in Migration.tsx.
  // NOTE: this relies on sessionStorage from the migration run — if Preview is
  // opened in a fresh session (e.g. via History later), this will read as false
  // even for a past Lakehouse deployment. A durable fix would store this flag
  // on the job record in the database instead.
  const wasLakehouseDeployment = !!sessionStorage.getItem("lakehouse_response");
  ...
  const handleScheduleRefresh = async () => {
    ...
    if (errors.length > 0) {
      ...
      toast({
        title: "Schedule updated",
        description: wasLakehouseDeployment
          ? "Both lakehouse and semantic model schedules have been configured."
          : "Semantic model refresh schedule has been configured.",
      });
      setScheduleOpen(false);
    }
    setScheduling(false);
  ...
  const handleRefreshNow = () => {
    ...
    toast({
      ...
      description: wasLakehouseDeployment
        ? "Both semantic model and lakehouse refreshes started successfully."
        : "Semantic model refresh started successfully.",
    });
          ...
          <div className="space-y-6 py-2">
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  {scheduleEnabled
                    ? wasLakehouseDeployment
                      ? "Both Lakehouse and Semantic Model refresh are active"
                      : "Semantic Model refresh is active"
                    : "Scheduled refresh is off"}
                </p>
              </div>
              <Switch checked={scheduleEnabled} onCheckedChange={setScheduleEnabled} />
