...
function PreMigrationAnalysisWithDiscovery() {
  ...
  const fetchDiscovery = async () => {
    ...
    try {
      ...
      // Persist detected connection details so Migration.tsx can show/edit
      // them in the connection-confirmation dialog before deployment starts.
      try {
        const connSummary = results.flatMap((r) =>
          (r.workbooks?.data_model?.databases || []).map((db: any) => ({
            name: db.name,
            server: db.server,
            type: db.type,
          })),
        ...
        if (connSummary.length > 0) {
          sessionStorage.setItem("discovery_connections", JSON.stringify(connSummary));
        }
      } catch (e) {
        console.error("Could not persist discovery connection details:", e);
      }
