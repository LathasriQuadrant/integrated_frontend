...
export default function Migration() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      updateStep(0, "running", "Extracting metadata…");
      try {
        // 1a) Extract metadata (moved first — gives us datasourceName before we upload the report)
        const metaRes = await fetch(
          `https://relationshipss-b3fbh7cehtfjghhr.eastus-01.azurewebsites.net/extract-metadata?folder_name=${encodeURIComponent(reportName)}`,
          { method: "POST" },
        );
        ...
        // Datasource name extracted server-side from the .twb (falls back to report name if absent)
        const datasourceName: string = metaData.datasourceName || reportName;
        sessionStorage.setItem("datasource_name", datasourceName);
        ...
        // 1c) Upload report — dataset (semantic model) named after the datasource, not the report
        const uploadRes = await fetch(
          "https://report-uploader-awa8avchh6gqa3ad.eastus-01.azurewebsites.net/upload-report",
          {
            method: "POST",
