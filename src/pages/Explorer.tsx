...
// ✅ Small helper so every token check logs the same shape of info.
const logTokenCheck = (label: string, token: string | null) => {
  console.log(`🔍 TOKEN CHECK in ${label}:`, {
    tokenExists: !!token,
    ...
    isNull: token === null,
    truthyCheck: !!token,
    failReason: !token ? "Failed truthiness check" : "Passed",
  });
};
...
const Explorer = () => {
  ...
  const handleRefresh = async () => {
    ...
    // ✅ ADDED LOGGING
    logTokenCheck("handleRefresh", token);

    if (!token) {
      console.error("❌ Token missing in handleRefresh - redirecting to dashboard");
      toast({
        title: "Session expired",
        description: "Please sign in again",
        variant: "destructive",
    ...
    try {
      ...
      // ✅ ADDED LOGGING
      console.log("🔍 fetch_data RESPONSE (handleRefresh):", {
  ...
  const runMigrationSteps = async (workbookId: string, workbookName: string) => {
    ...
    // ✅ ADDED LOGGING
    logTokenCheck("runMigrationSteps", token);

    if (!token) {
      console.error("❌ Token missing in runMigrationSteps - redirecting to dashboard");
      toast({ title: "Session expired", description: "Please sign in again", variant: "destructive" });
      navigate("/");
      return null;
    }
    ...
    // ✅ ADDED LOGGING
    console.log("🔍 download_workbook RESPONSE:", {
      status: dlRes.status,
      ok: dlRes.ok,
    });
  ...
  const handleMigrateWorkbook = async () => {
    ...
    try {
      ...
      // ✅ ADDED LOGGING
      console.error("❌ MIGRATE WORKBOOK ERROR:", error);
      const message = error instanceof Error ? error.message : "Migration preparation failed";
      toast({
        title: "Migration failed",
        description: message,
  ...
  const handleMigrateNode = async () => {
    ...
    try {
      ...
      // ✅ ADDED LOGGING
      console.error("❌ MIGRATE NODE ERROR:", error);
      const message = error instanceof Error ? error.message : "Migration preparation failed";
      toast({
        title: "Migration failed",
        description: message,
  ...
  const handleAnalyzeSelected = () => {
    ...
    // ✅ ADDED LOGGING
    logTokenCheck("handleAnalyzeSelected", token);

    if (!token) {
      console.error("❌ Token missing in handleAnalyzeSelected - redirecting to dashboard");
      toast({ title: "Session expired", description: "Please sign in again", variant: "destructive" });
      navigate("/");
      return;
    }
